...
import { CartItem, Product, ShippingOption, FreightShippingQuote } from "@/types";

interface CartContextType {
  items: CartItem[];
  addToCart: (product: Product, quantity?: number, shipping?: ShippingOption, freightShipping?: FreightShippingQuote) => void;
...
export function CartProvider({ children }: { children: React.ReactNode }) {
  ...
  const addToCart = (product: Product, quantity = 1, shipping?: ShippingOption, freightShipping?: FreightShippingQuote) => {
        ...
        return prevItems.map((item) =>
          item.product.id === product.id
            ? {
              ...
              freightShipping: freightShipping || item.freightShipping,
            }
            : item
        );
      }
